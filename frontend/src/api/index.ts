...
async function request<T = any>(path: string, opts: RequestInit & ApiRequestOptions = {}, raw = false): Promise<T> {
  return withRequestDeadline(opts, async signal => {
    ...
    if (!res.ok) {
      if (res.status === 401) {
        auth.logout(true)
        // 品牌定制：会话失效 → 独立登录页，带上当前路径登录后回来（上游是弹窗
        // 方案 → ?login=1，我们已改为 /login 路由）。已在登录页则跳过（登录
        // 接口密码错也回 401，不能引发重复跳转）；保留上游对 OAuth2 回调页
        // 与 /api/auth/me 探测的豁免。
        const h = window.location.hash
        if (path !== '/api/auth/me' && !h.startsWith('#/oauth2/callback') && !h.startsWith('#/login')) {
