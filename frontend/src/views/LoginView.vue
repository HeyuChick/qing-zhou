    ...
    <div class="login-card">
      <div class="login-brand">
        <div class="login-brand-inner">
          <div class="login-logo"><BrandMark :size="46" /></div>
          <div class="login-copy">
            <strong>{{ config.config.site_name || '黑羽短腿机场' }}</strong>
            <span>安全连接，从这里开始</span>
          </div>
        </div>
      </div>
...
// 登录/注册成功后的去向：优先回跳来源页（守卫/401 兜底带上来的 redirect），
// 只认站内路径（/ 开头且非 //），防外链跳转；否则默认控制台。
// 从独立页面导航过去，目标页是全新挂载，按登录态拉数据，无需整页刷新。
function goAfterAuth() {
  const r = typeof route.query.redirect === 'string' ? route.query.redirect : ''
  router.push(r.startsWith('/') && !r.startsWith('//') ? r : '/dashboard')
}
...
.login-page {
  ...
  padding: 24px;
  background:
    radial-gradient(circle at 18% 8%, rgba(71,155,207,.10), transparent 55%),
    radial-gradient(circle at 85% 92%, rgba(51,122,92,.08), transparent 50%),
    var(--bg, #f3f5f7);
}
.login-card {
  width: 100%;
  ...
  border: 1px solid rgba(28,48,70,.08);
  border-radius: 16px;
  box-shadow: 0 18px 50px rgba(28,48,70,.10);
  backdrop-filter: blur(24px) saturate(1.12);
  animation: loginIn .5s var(--ease-emphasized, ease-out) both;
}
...
.login-logo { width: 46px; height: 46px; display: grid; place-items: center; }
.login-copy { display: flex; flex-direction: column; line-height: 1.2; }
.login-copy strong { color: var(--text, #1d2733); font-size: 19px; font-weight: 700; letter-spacing: -.02em; }
.login-copy span { margin-top: 4px; color: var(--text-3, #8a97a6); font-size: 10.5px; font-weight: 500; }
.forgot-off {
  padding: 18px 14px; text-align: center; line-height: 1.8;
  font-size: 13px; color: var(--text-2, #4a5768);
  background: var(--bg-soft, #eef2f6); border-radius: 8px;
}
</style>
