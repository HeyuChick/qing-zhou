...
- **🧾 多套餐独立计费（桶模型）** —— 一个用户可同时持有多个套餐，每个套餐是独立的「桶」（各自流量 / 到期 / 节点 / 计量身份）；某套餐到期只下线它名下的节点，互不影响。
...
<!-- 建议在此处放置后台总览、节点管理、我的订阅等截图。例如：
![数据总览](docs/images/dashboard.png)
![我的订阅](docs/images/subscription.png)
-->

> 截图待补充。前端为单页应用，移动端自适应（侧栏转顶部、宽表横向滚动）。
...
3. **入站（节点）** —— 选协议建入站并绑定上一步的 TLS；同机可建多协议入站，订阅会自动聚成「锁定 IP·切协议」优选组。
