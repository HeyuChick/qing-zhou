...
- **📱 订阅多端自适应** —— 同一条链接按客户端 `User-Agent` 自动返回 **Clash / sing-box / Surge**，其余回退 base64；内置 **URLTest 智能优选**（全局选最快 / 锁定 IP 只切协议）与**防泄漏分流模板**（国内直连 / 广告拦截 / 其余代理），开箱即用。用浏览器打开订阅链接会看到一张**可读的信息页**（剩余流量 / 到期 / 节点数 + 一键复制），而不是一坨 base64。
...
- **🔒 安全内建** —— 敏感配置（SMTP 密码 / Reality 私钥等）库内 **AES-256-GCM 加密**；登录 / 注册 / 找回按 IP 限流；JWT 绑 `jti` 支持会话吊销；生成的节点配置**默认阻断私网出口**（内网段 / `127.0.0.1` / 云元数据 `169.254.169.254`），用户拿不到落地机的内网身份。
